...
Changelog:
  v20260406j - Added Cylon Centurion (BSG 1978) preset. The original voice
               used an ARP 2500 synthesizer + EMS Vocoder 1000 and was
               characterized by very low pitch, slow deliberate delivery,
               and measured word gaps. No espeak-ng preset can replicate
               the analog vocoder chain, but this approximates the acoustic
               character as closely as espeak-ng parameters allow.
...
def get_voices():
    ...
    parts[1] = voice code passed to -v  (e.g. "en-us")
    parts[4] = file path, basename used for label (e.g. "en/en-us" → "en-us")

    The en-us entry is always injected if absent so the combobox is never empty.
    """
