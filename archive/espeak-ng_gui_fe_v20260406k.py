...
Changelog:
  v20260406k - Restored full voice names (e.g. "English_(America) [en-us]")
               by reading the 'name' field from each voice file on disk
               rather than relying on the truncated --voices table output.
...
def _find_voice_data_dir():
    """Return the espeak-ng-data/voices/lang directory, or None if not found."""
    candidates = [
        "/usr/lib/x86_64-linux-gnu/espeak-ng-data/voices/lang",
        "/usr/lib/espeak-ng-data/voices/lang",
        "/usr/local/lib/espeak-ng-data/voices/lang",
        "/usr/share/espeak-ng-data/voices/lang",
        "/opt/homebrew/share/espeak-ng/voices/lang",          # macOS Homebrew
        os.path.expanduser("~/.espeak-ng-data/voices/lang"),
    ]
    # Also try locating via espeak-ng --voices output path hint
    try:
        r = subprocess.run(["espeak-ng", "--version"],
                           capture_output=True, text=True, timeout=3)
        for line in r.stdout.splitlines():
            if "Data at:" in line:
                data_dir = line.split("Data at:")[-1].strip()
                candidates.insert(0, os.path.join(data_dir, "voices", "lang"))
    except Exception:
        pass
...
def _read_voice_name(filepath):
    """Read the 'name' line from a voice file; return None if absent."""
    try:
...
def get_voices():
    ...
    Strategy:
      1. Run espeak-ng --voices to get the lang-code → file-path mapping.
      2. For each entry, open the voice file and read its 'name' field
         (e.g. "English_(America)") to build a human-readable label.
      3. Fall back to the file basename if the name field is missing.
    ...
    try:
        ...
        voice_dir = _find_voice_data_dir()
        voices = []
        for line in result.stdout.splitlines()[1:]:
            parts = line.split()
            if len(parts) < 5:
                continue
            lang      = parts[1]           # voice code, e.g. "en-us"
            file_path = parts[4]           # relative path, e.g. "en/en-us"

            # Try to read the full name from the voice file on disk
            full_name = None
            if voice_dir:
                abs_path = os.path.join(voice_dir, file_path)
                if os.path.isfile(abs_path):
                    full_name = _read_voice_name(abs_path)

            # Fall back to the file basename (e.g. "en-us")
            if not full_name:
        ...
        if not any(code == DEFAULT_VOICE_CODE for _, code in voices):
