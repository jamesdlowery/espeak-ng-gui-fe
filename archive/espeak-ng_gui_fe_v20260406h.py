...
# The guaranteed fallback entry that must always appear in the voice list.
...
def get_voices():
    ...
    espeak-ng --voices output columns (whitespace-separated):
      Pty  Lang       Age/Gender  VoiceName     File
      5    en-us      M           english-us    en/en-us
      5    en         M           default       default

    We use:
      - parts[1]  as the voice code passed to -v  (e.g. "en-us")
      - parts[4]  as the file path whose basename becomes the display name
                  (e.g. "en/en-us" → "en-us")

    The default en-us entry is always injected if not found in the output,
    so the combobox is never empty.
    """
    ...
    try:
        ...
        for line in result.stdout.splitlines()[1:]:   # skip header row
            ...
            file_path = parts[4]          # file path,   e.g. "en/en-us"
            basename  = os.path.basename(file_path)   # e.g. "en-us"
            # Build a readable label:  "en-us [en-us]"
            label = f"{basename} [{lang}]"
        ...
        # Ensure the en-us default is always present
...
class EspeakGUI:
    ...
    def _style_ttk(self):
        ...
        style.configure("TCombobox",
                        ...
                        lightcolor=PANEL,
                        darkcolor=PANEL)
        # Map covers active / focus / readonly / disabled states explicitly
        style.map("TCombobox",
