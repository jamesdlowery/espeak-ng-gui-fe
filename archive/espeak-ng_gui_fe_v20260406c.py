...
class EspeakGUI:
    ...
    def _build_ui(self):
        ...
        # After the window is drawn, widen the dropdown popup to fit the longest entry
    ...
    def _fix_combo_width(self):
        """Set the dropdown popup width to fit the longest voice name."""
        try:
            # Measure the longest entry in pixels using the combobox font
            font = tk_font.Font(font=self.voice_combo["font"])
            voice_names = [v[0] for v in self.voices]
            max_px = max(font.measure(n) for n in voice_names) + 20  # padding
            # Access the internal Listbox inside the Combobox popup
            self.voice_combo.tk.eval(
                f"[ttk::combobox::PopdownWindow {self.voice_combo}].f.l "
                f"configure -width 0"
            )
            self.voice_combo["width"] = max(
                max_px // font.measure("0"), 30
            )
        except Exception:
            pass  # Non-critical — falls back to fill="x" width
        style = ttk.Style()
